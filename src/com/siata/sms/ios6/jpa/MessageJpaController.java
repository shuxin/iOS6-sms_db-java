...
public class MessageJpaController implements Serializable {
	...
	public void create(Message message) throws PreexistingEntityException,
			Exception {
		...
		try {
			...
			if (chatMessageJoin != null) {
				chatMessageJoin = em.getReference(chatMessageJoin.getClass(),
			...
			for (Attachment attachmentListAttachmentToAttach : message
					.getAttachmentList()) {
				attachmentListAttachmentToAttach = em.getReference(
						attachmentListAttachmentToAttach.getClass(),
						attachmentListAttachmentToAttach.getRowid());
				attachedAttachmentList.add(attachmentListAttachmentToAttach);
			}
			message.setAttachmentList(attachedAttachmentList);
			em.persist(message);
			if (chatMessageJoin != null) {
				Message oldMessageOfChatMessageJoin = chatMessageJoin
						.getMessage();
				if (oldMessageOfChatMessageJoin != null) {
					oldMessageOfChatMessageJoin.setChatMessageJoin(null);
					oldMessageOfChatMessageJoin = em
							.merge(oldMessageOfChatMessageJoin);
				}
				chatMessageJoin.setMessage(message);
				chatMessageJoin = em.merge(chatMessageJoin);
			}
			for (Attachment attachmentListAttachment : message
					.getAttachmentList()) {
			...
			if (findMessage(message.getRowid()) != null) {
				throw new PreexistingEntityException("Message " + message
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	...
	public void edit(Message message) throws NonexistentEntityException,
			Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			Message persistentMessage = em.find(Message.class,
					message.getRowid());
			ChatMessageJoin chatMessageJoinOld = persistentMessage
					.getChatMessageJoin();
			ChatMessageJoin chatMessageJoinNew = message.getChatMessageJoin();
			List<Attachment> attachmentListOld = persistentMessage
					.getAttachmentList();
			List<Attachment> attachmentListNew = message.getAttachmentList();
			if (chatMessageJoinNew != null) {
				chatMessageJoinNew = em.getReference(
						chatMessageJoinNew.getClass(),
						chatMessageJoinNew.getMessageId());
			...
			for (Attachment attachmentListNewAttachmentToAttach : attachmentListNew) {
				attachmentListNewAttachmentToAttach = em.getReference(
						attachmentListNewAttachmentToAttach.getClass(),
						attachmentListNewAttachmentToAttach.getRowid());
				attachedAttachmentListNew
						.add(attachmentListNewAttachmentToAttach);
			}
			attachmentListNew = attachedAttachmentListNew;
			message.setAttachmentList(attachmentListNew);
			message = em.merge(message);
			if (chatMessageJoinOld != null
					&& !chatMessageJoinOld.equals(chatMessageJoinNew)) {
				chatMessageJoinOld.setMessage(null);
				chatMessageJoinOld = em.merge(chatMessageJoinOld);
			}
			if (chatMessageJoinNew != null
					&& !chatMessageJoinNew.equals(chatMessageJoinOld)) {
				Message oldMessageOfChatMessageJoin = chatMessageJoinNew
						.getMessage();
				if (oldMessageOfChatMessageJoin != null) {
					oldMessageOfChatMessageJoin.setChatMessageJoin(null);
					oldMessageOfChatMessageJoin = em
			...
			for (Attachment attachmentListOldAttachment : attachmentListOld) {
				if (!attachmentListNew.contains(attachmentListOldAttachment)) {
					attachmentListOldAttachment.getMessageList()
							.remove(message);
					attachmentListOldAttachment = em
							.merge(attachmentListOldAttachment);
				}
			}
			for (Attachment attachmentListNewAttachment : attachmentListNew) {
				if (!attachmentListOld.contains(attachmentListNewAttachment)) {
					...
					attachmentListNewAttachment = em
							.merge(attachmentListNewAttachment);
				}
			}
			em.getTransaction().commit();
		} catch (Exception ex) {
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findMessage(id) == null) {
					throw new NonexistentEntityException("The message with id "
							+ id + " no longer exists.");
				}
			}
			throw ex;
	...
	private List<Message> findMessageEntities(boolean all, int maxResults,
			int firstResult) {
