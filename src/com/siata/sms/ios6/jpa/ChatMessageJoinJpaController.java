...
public class ChatMessageJoinJpaController implements Serializable {
	...
	public void create(ChatMessageJoin chatMessageJoin)
		...
		try {
			...
			if (message != null) {
				ChatMessageJoin oldChatMessageJoinOfMessage = message
						.getChatMessageJoin();
				if (oldChatMessageJoinOfMessage != null) {
					oldChatMessageJoinOfMessage.setMessage(null);
					oldChatMessageJoinOfMessage = em
							.merge(oldChatMessageJoinOfMessage);
				}
			...
			if (findChatMessageJoin(chatMessageJoin.getMessageId()) != null) {
				throw new PreexistingEntityException("ChatMessageJoin "
						+ chatMessageJoin + " already exists.", ex);
			}
			throw ex;
		} finally {
			if (em != null) {
	...
	public void edit(ChatMessageJoin chatMessageJoin)
			throws NonexistentEntityException, Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			em.getTransaction().begin();
			ChatMessageJoin persistentChatMessageJoin = em.find(
					ChatMessageJoin.class, chatMessageJoin.getMessageId());
			...
			if (chatIdNew != null) {
				chatIdNew = em.getReference(chatIdNew.getClass(),
						chatIdNew.getRowid());
				chatMessageJoin.setChatId(chatIdNew);
			}
			if (messageNew != null) {
				messageNew = em.getReference(messageNew.getClass(),
						messageNew.getRowid());
			...
			if (messageNew != null && !messageNew.equals(messageOld)) {
				ChatMessageJoin oldChatMessageJoinOfMessage = messageNew
						.getChatMessageJoin();
				if (oldChatMessageJoinOfMessage != null) {
					oldChatMessageJoinOfMessage.setMessage(null);
					oldChatMessageJoinOfMessage = em
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findChatMessageJoin(id) == null) {
					throw new NonexistentEntityException(
							"The chatMessageJoin with id " + id
									+ " no longer exists.");
				}
			}
			throw ex;
	...
	public List<ChatMessageJoin> findChatMessageJoinEntities(int maxResults,
			int firstResult) {
		return findChatMessageJoinEntities(false, maxResults, firstResult);
	}

	private List<ChatMessageJoin> findChatMessageJoinEntities(boolean all,
