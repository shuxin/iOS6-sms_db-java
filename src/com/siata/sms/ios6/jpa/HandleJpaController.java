...
public class HandleJpaController implements Serializable {
	...
	public void create(Handle handle) throws PreexistingEntityException,
			Exception {
		...
		try {
			...
			for (Chat chatListChatToAttach : handle.getChatList()) {
				chatListChatToAttach = em.getReference(
						chatListChatToAttach.getClass(),
						chatListChatToAttach.getRowid());
			...
			if (findHandle(handle.getRowid()) != null) {
				throw new PreexistingEntityException("Handle " + handle
						+ " already exists.", ex);
			}
			throw ex;
		} finally {
	...
	public void edit(Handle handle) throws NonexistentEntityException,
			Exception {
		EntityManager em = null;
		try {
			em = getEntityManager();
			...
			for (Chat chatListNewChatToAttach : chatListNew) {
				chatListNewChatToAttach = em.getReference(
						chatListNewChatToAttach.getClass(),
						chatListNewChatToAttach.getRowid());
			...
			if (msg == null || msg.length() == 0) {
				...
				if (findHandle(id) == null) {
					throw new NonexistentEntityException("The handle with id "
							+ id + " no longer exists.");
				}
			}
			throw ex;
	...
	public void destroy(Integer id) throws NonexistentEntityException {
		...
		try {
			...
			try {
				...
				throw new NonexistentEntityException("The handle with id " + id
	...
	private List<Handle> findHandleEntities(boolean all, int maxResults,
			int firstResult) {
